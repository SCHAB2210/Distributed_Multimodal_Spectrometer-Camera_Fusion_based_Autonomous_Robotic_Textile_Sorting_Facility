...
# Load the models (they are Sequential models without defined inputs)
...
for layer in model1.layers:
    x = layer(x)
# The rebuilt model outputs a 64-dimensional feature vector.
rgb_feature_extractor = Model(inputs=rgb_input, outputs=x)

# ----- Rebuild Model 2 (NIR) as a functional model with explicit input -----
# For the NIR model, we exclude the last layer (assumed to be Dropout) and take the output from the Dense layer before it.
...
nir_features = nir_df.drop(columns=['label']).values  # (N, 105)
nir_labels = nir_df['label'].values  # (N,)

# ----- Remap NIR labels to match the RGB label ordering -----
# Original NIR labels: 0 = white, 1 = black, 2 = other
# Desired RGB ordering: 0 = black, 1 = other, 2 = white
...
# Update nir_labels with the remapped labels (if you plan to use them later)
nir_labels = nir_labels_remapped

# ----- Reshape NIR features to match model input shape: (N, 105, 1) -----
...
def extract_and_concatenate_features(rgb_batch, nir_batch):
    # Pass the batch through the functional models to extract features.
